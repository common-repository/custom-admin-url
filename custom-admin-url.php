<?php
/*
Plugin Name: Custom WP-Admin URL
Version: test
Plugin URI: http://mollik.me
Description: Small and simple plugin that allows you to change url of wp-admin
Author: sharif
Author URI: http://mollik.me
*/

if (!class_exists('CustomWPAdminURL')) {

	class CustomWPAdminURL {
		...
		function custom_admin_url() {
			...
			add_settings_field('custom_wpadmin_slug', 'WP-Admin URL', array($this, 'options_page'), 'permalink', 'optional', array('label_for' => 'custom_wpadmin_slug'));
	...
	$custom_wpadmin_url = new CustomWPAdminURL();

	// add hooks
	add_filter('generate_rewrite_rules', array($custom_wpadmin_url, 'rewrite_admin_url'));
	add_action('admin_init', array($custom_wpadmin_url, 'custom_admin_url'));
	add_action('login_init', array($custom_wpadmin_url, 'check_login'));
